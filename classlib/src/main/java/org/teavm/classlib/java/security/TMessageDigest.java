...
public abstract class TMessageDigest extends TMessageDigestSpi {
     ...
     */
    protected TMessageDigest(TString algorithm) {
    ...
    public static TMessageDigest getInstance(TString algorithm)
        ...
        try {
            ...
            if (objs[0] instanceof TMessageDigest) {
                ...
                md = new TMessageDigest.Delegate((TMessageDigestSpi)objs[0], algorithm);
            }
            ...
            throws NoSuchAlgorithmException, NoSuchProviderException
    {
        ...
        if (objs[0] instanceof java.security.MessageDigest) {
            ...
            TMessageDigest delegate =
                    new TMessageDigest.Delegate((TMessageDigestSpi)objs[0], algorithm);
    ...
    public final int getDigestLength() {
        ...
        if (digestLen == 0) {
            try {
                java.security.MessageDigest md = (java.security.MessageDigest)clone();
    ...
    static class Delegate extends TMessageDigest {

        // The provider implementation (delegate)
        private TMessageDigestSpi digestSpi;

        // constructor
        public Delegate(TMessageDigestSpi digestSpi, TString algorithm) {
            super(algorithm);
        ...
        public Object clone() throws CloneNotSupportedException {
            if (digestSpi instanceof Cloneable) {
                TMessageDigestSpi digestSpiClone =
                        (TMessageDigestSpi)digestSpi.clone();
                // Because 'algorithm', 'provider', and 'state' are private
                // members of our supertype, we must perform a cast to
                // access them.
                TMessageDigest that =
                        new TMessageDigest.Delegate(digestSpiClone,
                                ((TMessageDigest)this).algorithm);
                that.provider = ((TMessageDigest)this).provider;
                that.state = ((TMessageDigest)this).state;
                return that;
            } else {
                throw new CloneNotSupportedException();
            }
