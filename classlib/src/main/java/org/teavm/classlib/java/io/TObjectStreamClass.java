...
public class TObjectStreamClass extends TObject implements TSerializable {
     ...
     * TObjectStreamClass.lookup() (see 4461737)
     */
    ...
    private TObjectStreamClass.ExceptionInfo deserializeEx;
    /** exception (if any) to throw if non-enum serialization attempted */
    private TObjectStreamClass.ExceptionInfo serializeEx;
    /** exception (if any) to throw if default serialization attempted */
    private TObjectStreamClass.ExceptionInfo defaultSerializeEx;

    /** serializable fields */
    private TObjectStreamField[] fields;
    /** aggregate marshalled size of primitive fields */
    private int primDataSize;
    /** number of non-primitive fields */
    private int numObjFields;
    /** reflector for setting/getting serializable field values */
    private TObjectStreamClass.FieldReflector fieldRefl;
    /** data layout of serialized objects described by this class desc */
    private volatile TObjectStreamClass.ClassDataSlot[] dataLayout;
    ...
    private TObjectStreamClass localDesc;
    /** superclass descriptor appearing in stream */
    private TObjectStreamClass superDesc;
     ...
     * TObjectStreamClass instance if one does not exist yet for class. Null is
    ...
    /**
     * Return a string describing this TObjectStreamClass.
     */
     ...
     */
    TObjectStreamClass getSuperDesc() {
        requireInitialized();
        return superDesc;
    }
     ...
     * TObjectStreamClass.lookup(this.forClass())) or null if there is no class
     * associated with this descriptor.
     */
    TObjectStreamClass getLocalDesc() {
        requireInitialized();
        return localDesc;
    }
    ...
    static class ClassDataSlot {
        ...
        final TObjectStreamClass desc;
        /** true if serialized form includes data for this slot's descriptor */
        final boolean hasData;

        ClassDataSlot(TObjectStreamClass desc, boolean hasData) {
            this.desc = desc;
            ...
            throws InvalidClassException
    {
        ...
        for (TObjectStreamClass d = this; d != null; d = d.superDesc) {
            ...
            if (match != null) {
                for (Class<?> c = start; c != match; c = c.getSuperclass()) {
                    slots.add(new TObjectStreamClass.ClassDataSlot(
                            TObjectStreamClass.lookup(c, true), false));
                }
                start = match.getSuperclass();
            }

            // record descriptor/class pairing
            slots.add(new TObjectStreamClass.ClassDataSlot(d.getVariantFor(match), true));
        }
        ...
        return slots.toArray(new TObjectStreamClass.ClassDataSlot[slots.size()]);
    }
     ...
     */
    private TObjectStreamClass getVariantFor(Class<?> cl)
            throws InvalidClassException
    {
        if (this.cl == cl) {
            return this;
    ...
    /**
     * Returns TObjectStreamField array describing the serializable fields of
     ...
     * non-transient fields declared by given class.  Each TObjectStreamField
    ...
    private static long computeDefaultSUID(TClass<?> cl) {
        ...
        try {
            ...
            TObjectStreamClass.MemberSignature[] fieldSigs = new TObjectStreamClass.MemberSignature[fields.length];
            for (int i = 0; i < fields.length; i++) {
                fieldSigs[i] = new TObjectStreamClass.MemberSignature(fields[i]);
            }
            Arrays.sort(fieldSigs, new Comparator<TObjectStreamClass.MemberSignature>() {
                public int compare(
                        TObjectStreamClass.MemberSignature ms1, TObjectStreamClass.MemberSignature ms2) {
                    return ms1.name.compareTo(ms2.name);
                }
            });
            for (int i = 0; i < fieldSigs.length; i++) {
                TObjectStreamClass.MemberSignature sig = fieldSigs[i];
            ...
            TObjectStreamClass.MemberSignature[] consSigs = new TObjectStreamClass.MemberSignature[cons.length];
            for (int i = 0; i < cons.length; i++) {
                consSigs[i] = new TObjectStreamClass.MemberSignature(cons[i]);
            }
            Arrays.sort(consSigs, new Comparator<TObjectStreamClass.MemberSignature>() {
                public int compare(
                        TObjectStreamClass.MemberSignature ms1, TObjectStreamClass.MemberSignature ms2) {
            ...
            for (int i = 0; i < consSigs.length; i++) {
                TObjectStreamClass.MemberSignature sig = consSigs[i];
                int mods = sig.member.getModifiers() &
                        (Modifier.PUBLIC | Modifier.PRIVATE | Modifier.PROTECTED |
            ...
            TObjectStreamClass.MemberSignature[] methSigs = new TObjectStreamClass.MemberSignature[methods.length];
            for (int i = 0; i < methods.length; i++) {
                methSigs[i] = new TObjectStreamClass.MemberSignature(methods[i]);
            }
            Arrays.sort(methSigs, new Comparator<TObjectStreamClass.MemberSignature>() {
                public int compare(
                        TObjectStreamClass.MemberSignature ms1, TObjectStreamClass.MemberSignature ms2) {
            ...
            for (int i = 0; i < methSigs.length; i++) {
                TObjectStreamClass.MemberSignature sig = methSigs[i];
                int mods = sig.member.getModifiers() &
                        (Modifier.PUBLIC | Modifier.PRIVATE | Modifier.PROTECTED |
                                Modifier.STATIC | Modifier.FINAL |
                                Modifier.SYNCHRONIZED | Modifier.NATIVE |
            ...
            throws InvalidClassException
    {
        ...
        Class<?> cl = (localDesc != null && fields.length > 0) ?
                localDesc.cl : null;
        processQueue(TObjectStreamClass.Caches.reflectorsQueue, TObjectStreamClass.Caches.reflectors);
        TObjectStreamClass.FieldReflectorKey key = new TObjectStreamClass.FieldReflectorKey(cl, fields,
                TObjectStreamClass.Caches.reflectorsQueue);
        Reference<?> ref = TObjectStreamClass.Caches.reflectors.get(key);
        Object entry = null;
        if (ref != null) {
            entry = ref.get();
        }
        ...
        if (entry == null) {
            ...
            do {
                if (ref != null) {
                    TObjectStreamClass.Caches.reflectors.remove(key, ref);
                }
                ref = TObjectStreamClass.Caches.reflectors.putIfAbsent(key, newRef);
                if (ref != null) {
                    entry = ref.get();
                }
            } while (ref != null && entry == null);
        ...
        if (entry instanceof TObjectStreamClass.FieldReflector) {  // check common case first
            return (TObjectStreamClass.FieldReflector) entry;
        } else if (entry instanceof TObjectStreamClass.EntryFuture) {
            entry = ((TObjectStreamClass.EntryFuture) entry).get();
        } else if (entry == null) {
            try {
                entry = new TObjectStreamClass.FieldReflector(matchFields(fields, localDesc));
            } catch (Throwable th) {
                entry = th;
            }
            future.set(entry);
            TObjectStreamClass.Caches.reflectors.put(key, new SoftReference<Object>(entry));
        }

        if (entry instanceof TObjectStreamClass.FieldReflector) {
            return (TObjectStreamClass.FieldReflector) entry;
    ...
    private static class FieldReflectorKey extends WeakReference<Class<?>> {
        ...
        public boolean equals(Object obj) {
            ...
            if (obj instanceof TObjectStreamClass.FieldReflectorKey) {
                TObjectStreamClass.FieldReflectorKey other = (TObjectStreamClass.FieldReflectorKey) obj;
                Class<?> referent;
     ...
     * which each TObjectStreamField whose signature matches that of a local
            ...
            throws InvalidClassException
    {
         ...
         * TObjectStreamField.getType() returned Object.class if the
         * TObjectStreamField represented a non-primitive field and belonged to
         * a non-local class descriptor.  To preserve this (questionable)
         * behavior, the TObjectStreamField instances returned by matchFields
    ...
    static class WeakClassKey extends WeakReference<Class<?>> {
        ...
        public boolean equals(Object obj) {
            ...
            if (obj instanceof TObjectStreamClass.WeakClassKey) {
                Object referent = get();
                return (referent != null) &&
                        (referent == ((TObjectStreamClass.WeakClassKey) obj).get());
            } else {
                return false;
            }
        }
